...
const gcrevoke = ["6282221792667-1621430788@g.us"]
prefix = '#'
shp = '⬡'
owner = "6282221792667@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["Terhubung", "BOTCAHBOTz", "3.0"];
  ...
  client.on("qr", () => {
    console.log(
      ...
      client.sendMessage(`6282221792667@s.whatsapp.net`, `「 *NOTIFICATION!* 」\n\n _Bot Connected Successfully!_`, MessageType.extendedText)
  ...
  client.on("open", () => {
    success("2", "Successfully connected to wa, Donation");
console.log(color(figlet.textSync(`${spc1}            Bot`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','red'), color('𝙄 𝙬𝙧𝙤𝙩𝙚 𝙩𝙝𝙞𝙨 𝙨𝙘𝙧𝙞𝙥𝙩 𝙢𝙮𝙨𝙚𝙡𝙛!\n𝙉𝙤𝙩𝙚: 𝘿𝙤𝙣𝙩 𝙛𝙤𝙧𝙜𝙚𝙩 𝙩𝙤 Donation', 'yellow'))
    console.log(color('⎇','red'), color('𝗦𝗼𝘂𝗿𝗰𝗲 𝗖𝗼𝗱𝗲 𝗩𝗲𝗿𝘀𝗶𝗼𝗻:', 'aqua'), color(version))
    console.log(color('⎇','red'), color('𝗕𝘂𝗴? 𝗘𝗿𝗿𝗼𝗿? 𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗶𝗼𝗻? 𝗩𝗶𝘀𝗶𝘁 𝗛𝗲𝗿𝗲:', 'aqua'), color('https://wa.me/6282221792667'))
    console.log(color('[DOGE BOT]'), color('BOT Sudah Online ㋛︎', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 teks = `https://chat.whatsapp.com/JEDQfzMGZan4HA98wtxsNL`
 client.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Joined to Forum Whatsapp Bot group', 'cyan'))
 client.sendMessage(`6282221792667@s.whatsapp.net`, `*Hi Owner Bot, the bot has been successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "SERVER TIO INC CHROME.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/6282221792667?text=Hello bro"}}})
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Hi, Saya BOTWEA Siap Membantu Anda:)", "conversation");
      }
...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
      ...
      if (anu.action == "add" && !mem.includes(client.user.jid)) {
        ...
        buttons = [
          { buttonId: `verify`, buttonText: { displayText: "𝑽𝒆𝒓𝒊𝒇𝒚" }, type: 1 },
          { buttonId: `command`, buttonText: { displayText: "𝑪𝒍𝒊𝒄𝒌 𝑴𝒆𝒏𝒖" }, type: 1 },
        ];
        imageMsg = (
      ...
      if (anu.action == "remove" && !mem.includes(client.user.jid)) {
        ...
        buttons = [
          { buttonId: `y`, buttonText: { displayText: "𝘉𝘺𝘦𝘦" }, type: 1 },
